import { Outlet, useLocation, useNavigate, useLoaderData, useRouteLoaderData } from "react-router";
    ...
    <div className="p-4">
      <div className="flex flex-row items-center gap-3">
        <Avatar className="size-12">
          ...
          <AvatarFallback>{(user.displayUsername ?? user.name ?? "?").charAt(0).toUpperCase()}</AvatarFallback>
        </Avatar>
        <div className="min-w-0">
          <h1 className="truncate text-3xl font-bold">{user.displayUsername ?? user.name ?? "User"}</h1>
          {user.name && user.displayUsername && user.name !== user.displayUsername ? (
